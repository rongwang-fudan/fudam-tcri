% 2020-2100
% 用punits填充IIASA中可再生能源，计算其真实投资。
% carbon tax revenue有多少留作下一年用（100%）
...
%%
load('H:\Transfer of carbon-tax revenue\inputs\ScenarioID_Type.mat');
...
load('H:\Transfer of carbon-tax revenue\inputs\CO2_Energy_IIASA.mat')
% Emissions|CO2, Mt CO2/yr
enepro(:,:,:,11)=CO2_Energy_IIASA(1:12,21:5:101,:); clear CO2_Energy_IIASA;
load('H:\Transfer of carbon-tax revenue\inputs\GDP_PPP_IIASA.mat')
% GDP|PPP, billion US$2010/yr
enepro(:,:,:,12)=GDP_PPP_IIASA(1:12,21:5:101,:); clear GDP_PPP_IIASA;
...
%     end
%     n3=n3+1;
%     a=sum(enepro(:,:,sce,1:3),4);
%     subplot(2,2,1); plot(a(2,:),'LineStyle','-','LineWidth',0.1,'Color',linecolor(ScenarioID_Type(sce,2),1:3)); hold on;
%     subplot(2,2,2); plot(a(5,:),'LineStyle','-','LineWidth',0.1,'Color',linecolor(ScenarioID_Type(sce,2),1:3)); hold on;
%     subplot(2,2,3); plot(a(7,:),'LineStyle','-','LineWidth',0.1,'Color',linecolor(ScenarioID_Type(sce,2),1:3)); hold on;
%     subplot(2,2,4); plot(a(8,:),'LineStyle','-','LineWidth',0.1,'Color',linecolor(ScenarioID_Type(sce,2),1:3)); hold on;
% end
...
for exp=1%:1
    ...
    for sss = 1:size(mbmb,1)
        ...
        %         % interpolation of global energy and co2 emissions to regions for scenarios without data using a profile from a scenario
        %         for t=1:17
        %             for j=1:12
        %                 for i=2:12
        %                     if isnan(enepro(i,t,sce,j))
        %                         enepro(i,t,sce,j)=enepro(1,t,sce,j)*energyprofile(i,j)/energyprofile(1,j);
        %                     end
        %                 end
        %                 % scaling the regional sum to be equal to the global total
        %                 if sum(enepro(2:12,t,sce,j),1)~=0
        %                     rscale=enepro(1,t,sce,j)/sum(enepro(2:12,t,sce,j),1);
        %                     enepro(2:12,t,sce,j)=enepro(2:12,t,sce,j).*rscale;
        %                 end
        %             end
        %         end
                            ...
                            for k=1:size(idx,1)
                                if te+rrr(idx(k),1)*punits(idx(k),5)<=an
                                    ...
                                    rrr(idx(k),1) = 0;
                                        ...
                                        enepro22(i,t:end,sce,j+3) = enepro22(i,t:end,sce,j+3) + an-te;
                                        te=an; % EJ/yr
                                        %                             break;
                                    end
                                end
        ...
        for cp=17%1:16
            ...
            t = 1;
            for eerre = 1:6
%                 [m,n]=find(punits(:,2)==eerre & idyear(:,1)==t*5+2015);
                [m,n]=find(punits(:,2)==eerre & rrr_finer(:,t)~=0);
                costRE_iiasa(t,eerre+3,sce)=sum(punits(m,21).*rrr_finer(m,t)); % total initial investment m$
                for reggg = 2:12
                [m,n]=find(punits(:,23)==reggg & punits(:,2)==eerre & rrr_finer(:,t)~=0);
                costRE_iiasa_reg(t,reggg,eerre+3,sce)=sum(punits(m,21).*rrr_finer(m,t)); % total initial investment m$
                end
            end
            ...
            for t=2:17
                ...
                if t==2
                    ...
                    for eerre = 1:6
                        [m,n]=find(punits(:,2)==eerre & rrr_finer(:,t)~=0);
                        costRE_iiasa(t,eerre+3,sce)=sum(punits(m,21).*rrr_finer(m,t))*costlearning(eerre); % total initial investment m$
                        for reggg = 2:12
                            [m,n]=find(punits(:,23)==reggg & punits(:,2)==eerre & rrr_finer(:,t)~=0);
                            costRE_iiasa_reg(t,reggg,eerre+3,sce)=sum(punits(m,21).*rrr_finer(m,t))*costlearning(eerre); % total initial investment m$
                        end
                ...
                for i=2:12
                    ...
                    pff(t,i,1)=enepro(i,t,sce,1)-enepro(i,1,sce,1)*max(0,(1-(t-1)*5/fflifetime));
                    pff(t,i,2)=enepro(i,t,sce,2)-enepro(i,1,sce,2)*max(0,(1-(t-1)*5/fflifetime));
                    pff(t,i,3)=enepro(i,t,sce,3)-enepro(i,1,sce,3)*max(0,(1-(t-1)*5/fflifetime));
                    ...
                    pff(t,i,1)=max(0,pff(t,i,1)-enepro(i,1,sce,1)*frac_unsub); % final energy coal EJ/yr
                    pff(t,i,2)=max(0,pff(t,i,2)-enepro(i,1,sce,2)*frac_unsub); % final energy oil EJ/yr
                    pff(t,i,3)=max(0,pff(t,i,3)-enepro(i,1,sce,3)*frac_unsub); % final energy gas EJ/yr
                    ...
                    if cp>1 && t>=3
                        ...
                        if size(idx,1)>0 && ctr(t-1,i)>0 && decommissions==0
                            j=1;
                            while ctr(t-1,i)>(punits(idx(j),21)*rrr(idx(j))*costlearning(punits(idx(j),2)))
                                ...
                                idyear(idx(j),2)=(t-1)*5+2015; % year of building this power unit
                                ctr(t-1,i)=ctr(t-1,i)-punits(idx(j),21)*rrr(idx(j))*costlearning(e); % consume carbon tax revenue million $
                                rrr(idx(j))=0;
                                if j<size(idx,1)
                                    j=j+1;
                                else
                ...
                if financeexchange==1 && cp>0 && t>=3
                    ctr(t-1,1)=ctr(t-2,1)+sum(ctr(t-1,2:12),2); % the remaining carbon tax revenue becomes global finance million $
                    ctr(t-1,2:12)=ctr(t-1,2:12).*0; % the remaining carbon tax revenue becomes zero in each region
                    ...
                    if size(idx,1)>0 && ctr(t-1,1)>0
                        j=1;
                        while ctr(t-1,1)>(punits(idx(j),21)*rrr(idx(j))*costlearning(punits(idx(j),2)))
                            i=punits(idx(j),23);
                            if i==0
                                if j<size(idx,1)
                            ...
                            ctr(t-1,1)=ctr(t-1,1)-(punits(idx(j),21)*rrr(idx(j))*costlearning(e)); % remaining carbon tax revenue million $
                            if j<size(idx,1)
                                j=j+1;
                            else
                                break;
                ...
                for i=2:12
                    ...
                    co2path(1,t,cp,sce)=co2path(1,t,cp,sce)+co2path(i,t,cp,sce); % Gt CO2/yr
...
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_invREiiasa_sce_reg_finalenergy0424.mat','invREiiasa_sce_reg','-v7.3'); % million/y
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_invREpath_sce_reg_finalenergy0424.mat','invREpath_sce_reg','-v7.3');

save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_invREpath_sce_noiiasa_finalenergy0424.mat','invREpath_sce','-v7.3');

save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_invREiiasa_sce_ori_finalenergy0424.mat','invREiiasa_sce_ori','-v7.3'); % million/y
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_invREpath_sce_ori_finalenergy0424.mat','invREpath_sce_ori','-v7.3');
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_invREiiasa_sce_finalenergy0424.mat','invREiiasa_sce','-v7.3'); % million/y
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_invREpath_sce_finalenergy0424.mat','invREpath_sce','-v7.3');
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_co2path_sce_finalenergy0424.mat','co2path_sce','-v7.3');
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_energyREpath_sce_finalenergy0424.mat','energyREpath_sce','-v7.3');
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_energyALLpath_sce_finalenergy0424.mat','energyALLpath_sce','-v7.3');

energyREpath_sce_iiasa = (reshape(sum(sum(enepro22(2:12,1:7,:,4:9),1),4),[7,1291]))'/3.6;
save('H:\Transfer of carbon-tax revenue\Ans\fig2_iiasa_energyREpath_sce_iiasa_finalenergy0424.mat','energyREpath_sce_iiasa','-v7.3');
