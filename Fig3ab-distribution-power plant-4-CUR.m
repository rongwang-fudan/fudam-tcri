...
punits_CUR = punits(idxxx_2020to2200_CUR(:,3),:);
[m,n] = find(punits_CUR(:,5)/3.6>10);
[m,n] = find(punits_CUR(:,5)/3.6>10 & punits_CUR(:,3)>2);
sum(punits_CUR(:,15))*100/10^6 % Mha
...
% 12.county; 13.country; 14.CP(MW); 15.km2
for reg = 2:12
    [m,n] = find(punits_CUR(:,4)==reg);
    aaaaa0(reg,1) = sum(punits_CUR(m,5))/3.6;
end
load('H:\Transfer of carbon-tax revenue\Ans\Power_renewable_CUR.mat','Power_renewable_CUR');
aaaaa = Power_renewable_CUR(31,:)';
r = aaaaa(2:12)./aaaaa0(2:12);
for reg = 2:12
    [m,n] = find(punits_CUR(:,4)==reg);
    punits_CUR(m,5) = punits_CUR(m,5)*r(reg-1);
end


load('H:\Transfer of carbon-tax revenue\Ans\Power_all_CUR.mat','Power_all_CUR');
...
[ttti Power_all_CUR(31,:)']
Power_renewable_CUR(31,:)'./Power_all_CUR(31,:)'



%
[m,n]=find(punits_CUR(:,3)<=2);
punits_pw = punits_CUR(m,:);
[m,n]=find(punits_CUR(:,3)>2);
punits_others = punits_CUR(m,:);

[m,n]=find(punits_CUR(:,3)==3);
punits_bioenergy = punits_CUR(m,:);
[m,n]=find(punits_CUR(:,3)==4);
punits_nuclear = punits_CUR(m,:);
[m,n]=find(punits_CUR(:,3)==5);
punits_hydropower = punits_CUR(m,:);
[m,n]=find(punits_CUR(:,3)==6);
punits_geothermal = punits_CUR(m,:);
[m,n]=find(punits_CUR(:,3)==7);
punits_CCS = punits_CUR(m,:);
...
save('H:\Transfer of carbon-tax revenue\Ans\renewable_power_pant_CUR_posifix.mat','renewable_power_pant_fix','-v7.3');

renewable_power_pant = aaaf22;
save('H:\Transfer of carbon-tax revenue\Ans\renewable_power_pant_CUR_no.mat','renewable_power_pant','-v7.3');
% 2.dynamic year; 3.type (1 solar, 2 wind, 3 biomass, 4 nuclear, 5 hydropower, 6 geothermal, 7 CCS);
% 4.region id; 5.power generation PJ/yr; 6.total investment t$
% 7.abated emission GtCO2/yr; 8.fraction of non-land costs in total costs;
% 9.latitude; 10.longitude; 11.marginal abatement cost $/tCO2;
...
tic
clear;
load('H:\Transfer of carbon-tax revenue\Ans\renewable_power_pant_CUR_no.mat','renewable_power_pant');
% 2.dynamic year; 3.type (1 solar, 2 wind, 3 biomass, 4 nuclear, 5 hydropower, 6 geothermal, 7 CCS);
% 4.region id; 5.power generation PJ/yr; 6.total investment t$
% 7.abated emission GtCO2/yr; 8.fraction of non-land costs in total costs;
% 9.latitude; 10.longitude; 11.marginal abatement cost $/tCO2;
...
figure;plot(ans)
renewable_power_pant_CUR_posinofix = renewable_power_pant_a;
save('H:\Transfer of carbon-tax revenue\Ans\renewable_power_pant_CUR_posinofix.mat','renewable_power_pant_CUR_posinofix','-v7.3');
